...
class WeinerFilter(object):
    ...
    def __init__(self):
        self.image_path = r"PythonFiles\car.jpg"
    ...
    def pad_image(self):
        ...
        pad_y, pad_x = int(
            self.image.shape[0] - c_y), int(self.image.shape[1] - c_x)
        self.padded_image = np.pad(
            orig_img, ((0, pad_y), (0, pad_x), (0, 0)), constant_values=(255))

    def calculate_power_spectral_density(self):

        self.SB = (np.absolute(
            fftshift(fft2(self.padded_image[..., 0])))**2)/self.gamma
        self.SG = (np.absolute(
            fftshift(fft2(self.padded_image[..., 1])))**2)/self.gamma
        self.SR = (np.absolute(
            fftshift(fft2(self.padded_image[..., 2])))**2)/self.gamma
    ...
    def clean_image(self):
        self.output_image = cv2.fastNlMeansDenoisingColored(
            self.output_image, None, 10, 7, 3)
    ...
    def apply_weiner_filter(self):
        ...
        self.output_image[..., 0] = self.deblur_image(
            self.image[..., 0], self.kernel, self.S[0])
        self.output_image[..., 1] = self.deblur_image(
            self.image[..., 1], self.kernel, self.S[1])
        self.output_image[..., 2] = self.deblur_image(
            self.image[..., 2], self.kernel, self.S[2])
    ...
    def remove_ringing_effect(self):
        ...
        self.output_image = cv2.convertScaleAbs(
            self.output_image, alpha=2.5, beta=3)
